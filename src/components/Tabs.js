...
const TabsNavigation = (props) => {
  ...
  return <nav className={`tab-nav ${className}`}>{children}</nav>;
};
...
const TabsNavigationItem = (props) => {
  ...
  const activeClass = !pathname.includes("organizations")
    ? {
        active: isActive(),
      }
    : {};
  ...
  return (
    <button
      className={classNames("nav-link", "h5", className, activeClass)}
  ...
  return (
    <SwitchTransition>
      ...
      >
        <div
          ref={tabContentRef}
          className={classNames("open", "mt-5", className)}
        >
          {renderItems()}
        </div>
      </CSSTransition>
